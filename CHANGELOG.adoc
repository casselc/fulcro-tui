...
* Main-axis justification: `vbox`/`hbox` accept a `:justify` attribute (`:start` (default), `:center`, `:end`, `:space-between`, `:space-around`, `:space-evenly`) that positions children along the main axis when they don't fill it — the container-level counterpart to per-child cross-axis `:align`. `place-stack` gains a 4-arity carrying it.
