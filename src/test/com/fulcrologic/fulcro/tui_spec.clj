...
(specification {:covers {`tui/node? "277a4d,ef6fa0"}} "node?"
               (assertions
...
(specification {:covers {`tui/element "0b6694,464bda"}} "element generators"
               ...
               (component "modal"
                          (assertions
                           ...
                           (tui/node-attr (tui/modal {:id :dlg :open? true}) :id) => :dlg
                           (tui/node-attr (tui/modal {:id :dlg :open? true}) :open?) => true
                           "defaults :border? to true"
                           (tui/node-attr (tui/modal {:id :d}) :border?) => true
                           "lets a caller override the border default"
                           (tui/node-attr (tui/modal {:id :d :border? false}) :border?) => false
                           "keeps its children (stacked like a vbox)"
                           (mapv ::tui/tag (::tui/children (tui/modal {:id :d} (tui/text {} "hi")))) => [:text])))
...
(specification {:covers {`tui/wrapping-text? "e01c56,75544d"}} "wrapping-text?"
               (assertions
...
(specification {:covers {`tui/intrinsic-size "df71b2,d7562d"}} "intrinsic-size"
               (component "leaves"
                          (assertions
...
(specification {:covers {`tui/place "b3e3fa,e5243f"}} "place"
...
(specification {:covers {`tui/viewport? "014fb0,75544d"}} "viewport?"
               (assertions
...
(specification {:covers {`tui/placed-viewports "ade25c,96f37e"
                         `tui/focus-viewport-context "e6a13e,ca8114"}} "placed-viewports / focus-viewport-context"
...
(specification {:covers {`tui/render-buffer "e06fec,d0d0e6"
                         `tui/paint        "9cb0a6,ac1e59"}} "render-buffer / paint"
...
(specification {:covers {`tui/frame->ansi "9eafc2,5c8058"}} "frame->ansi — clear-screen on resize"
               (let [b1  (tui/make-buffer 1 5)
                 ...
                 (assertions
                  ...
                  (str/starts-with? (tui/frame->ansi b1 nxt {:clear? true}) "[2J[H") => true
                  "emits no clear-screen when :clear? is false"
                  (str/includes? (tui/frame->ansi b1 nxt {:clear? false}) "[2J") => false
                  "emits no clear-screen when :clear? is absent"
                  (str/includes? (tui/frame->ansi b1 nxt {}) "[2J") => false
                  "places the clear INSIDE the sync wrapper when both are set (atomic clear+repaint)"
                  (str/starts-with? (tui/frame->ansi b1 nxt {:sync? true :clear? true}) "[?2026h[2J") => true)))
...
(specification {:covers {`tui/render-tree "4667e2,5c2dd7"}} "render-tree"
...
(specification {:covers {`tui/render-root "156754,c1c98b"}} "render-root"
...
(specification {:covers {`tui/node-attr "d53a76,75544d"}} "node-attr"
               (assertions
...
(specification {:covers {`tui/find-by-id "3fafe1,8ae121"}} "find-by-id"
               (let [tree (tui/vbox {:id "root"}
                                    (tui/text {:id "a"} "A")
                                    (tui/hbox {:id "mid"}
...
(specification {:covers {`tui/node-text "752bdf,75544d"}} "node-text"
               (let [tree (tui/vbox {}
                                    (tui/text {} "Hello ")
...
(specification {:covers {`tui/activate! "99f954,8ae121"}} "activate!"
...
(specification {:covers {`tui/press! "f6f187,8ae121"}} "press!"
               (let [seen (atom nil)
...
(specification {:covers {`tui/type! "e574a3,8ae121"}} "type!"
               (let [typed (atom nil)
...
(specification {:covers {`tui/multiline-input? "6f29cd,8ae121"}} "multiline-input?"
               (assertions
...
(specification {:covers {`tui/handle-input-key! "11ab98,caa37d"}} "handle-input-key! (single-line vs multiline)"
...
(specification {:covers {`tui/focusable-node? "578cb2,8ae121"}} "focusable-node?"
               (assertions
...
(specification {:covers {`tui/focusables       "af1f15,f9541e"
                         `tui/focus-order      "ce7895,60de49"
...
(specification {:covers {`tui/apply-focus-change! "14481f,804804"}} "apply-focus-change!"
               (let [evts (atom [])
...
(specification {:covers {`tui/route-key "d7b983,806419"}} "route-key"
...
(specification {:covers {`tui/process-key! "a34332,714f22"}} "process-key!"
...
(specification {:covers {`tui/modal-node? "01af9e,75544d"}} "modal-node?"
               (assertions
                "is true for a :modal node"
                (tui/modal-node? (tui/modal {:id :d})) => true
                "is false for other nodes"
                (tui/modal-node? (tui/vbox {})) => false
                (tui/modal-node? (tui/button {:id :b} "x")) => false
                "is false for non-nodes"
                (tui/modal-node? {:a 1}) => false
                (tui/modal-node? nil) => false))

(let [tree (tui/vbox {:id "root"}
                     (tui/button {:id :base} "Base")
                     (tui/modal {:id :open-1 :open? true} (tui/button {:id :o1} "O1"))
                     (tui/modal {:id :closed :open? false} (tui/button {:id :c} "C"))
                     (tui/modal {:id :open-2 :open? true} (tui/button {:id :o2} "O2")))]

  (specification {:covers {`tui/collect-overlays "a78c8f,2a7273"}} "collect-overlays"
                 (assertions
                  "returns only the OPEN modal nodes (closed ones omitted)"
                  (mapv #(tui/node-attr % :id) (tui/collect-overlays tree)) => [:open-1 :open-2]
                  "returns them in document (pre-order) order, so the last is topmost"
                  (tui/node-attr (peek (tui/collect-overlays tree)) :id) => :open-2
                  "returns an empty vector when there are no open modals"
                  (tui/collect-overlays (tui/vbox {} (tui/button {:id :b} "x"))) => []))

  (specification {:covers {`tui/strip-overlays "5c4c5b,c25b8c"}} "strip-overlays"
                 (let [stripped (tui/strip-overlays tree)]
                   (assertions
                    "removes every :modal child (open or closed), keeping the rest"
                    (mapv ::tui/tag (::tui/children stripped)) => [:button]
                    "leaves no modal contents focusable in the base tree"
                    (mapv :id (tui/focusables stripped)) => [:base])))

  (specification {:covers {`tui/active-tree "976472,93d1d8"}} "active-tree"
                 (assertions
                  "returns the topmost OPEN modal as the focus-trap subtree"
                  (tui/node-attr (tui/active-tree tree) :id) => :open-2
                  "exposes only that modal's controls as focusable"
                  (mapv :id (tui/focusables (tui/active-tree tree))) => [:o2]
                  "with no open modal, returns the base tree with closed modals stripped"
                  (mapv :id (tui/focusables
                             (tui/active-tree (tui/vbox {:id "r"}
                                                        (tui/button {:id :base} "B")
                                                        (tui/modal {:id :c :open? false}
                                                                   (tui/button {:id :hidden} "H"))))))
                  => [:base])))

(specification {:covers {`tui/overlay-window-rect "1c450f,2e15d4"}} "overlay-window-rect"
               (assertions
                "centers a fixed-size window within the screen by default"
                (tui/overlay-window-rect (tui/modal {:id :d :width 10 :height 4}) {:x 0 :y 0 :w 20 :h 8})
                => {:x 5 :y 2 :w 10 :h 4}
                "resolves a fractional width/height against the screen"
                (tui/overlay-window-rect (tui/modal {:id :d :width [:fraction 0.5] :height [:fraction 0.5]})
                                         {:x 0 :y 0 :w 40 :h 20})
                => {:x 10 :y 5 :w 20 :h 10}
                "clamps the window to the screen size"
                (tui/overlay-window-rect (tui/modal {:id :d :width 100 :height 100}) {:x 0 :y 0 :w 20 :h 8})
                => {:x 0 :y 0 :w 20 :h 8}
                "honors :align :start (top-left)"
                (tui/overlay-window-rect (tui/modal {:id :d :width 10 :height 4 :align :start}) {:x 0 :y 0 :w 20 :h 8})
                => {:x 0 :y 0 :w 10 :h 4}))

(specification {:covers {`tui/paint "9cb0a6,ac1e59"
                         `tui/place "b3e3fa,e5243f"}} "modal layout & paint"
               (let [m       (tui/modal {:id :d :title "Menu" :width 10 :height 4} (tui/text {} "hi"))
                     placed  (tui/place m {:x 2 :y 1 :w 10 :h 4})
        ;; paint a full-screen base first, then the modal on top, to prove opacity.
                     base    (tui/render-buffer (tui/place (tui/text {} (apply str (repeat 14 \X))) {:x 0 :y 0 :w 14 :h 6}) 6 14)
                     buf     (tui/paint base placed {:x 0 :y 0 :w 14 :h 6})
                     scr     (tui/screen buf)]
                 (assertions
                  "places the modal at the given window rect"
                  (::tui/rect placed) => {:x 2 :y 1 :w 10 :h 4}
...
(specification {:covers {`tui/picker "dd855e,57d045"}} "picker"
               (let [selected (atom nil)
                     cancelled (atom false)
                     p (tui/picker {:id :fruit :open? true :title "Fruit" :width 16 :height 6
                                    :options [{:value :apple :label "Apple"}
                                              {:value :pear :label "Pear"}]
                                    :on-select (fn [v] (reset! selected v))
                                    :on-cancel (fn [] (reset! cancelled true))})
                     viewport (first (::tui/children p))
                     list-box (first (::tui/children viewport))
                     rows     (::tui/children list-box)]
                 (assertions
                  "is a modal carrying the picker's id, title, and size"
                  (::tui/tag p) => :modal
                  (tui/node-attr p :id) => :fruit
                  (tui/node-attr p :title) => "Fruit"
                  "wraps its rows in a scrollable viewport keyed off the picker id"
                  (::tui/tag viewport) => :viewport
                  (tui/node-attr viewport :id) => :fruit-list
                  "renders one focusable button row per option, in order"
                  (mapv ::tui/tag rows) => [:button :button]
                  (mapv #(tui/node-attr % :id) rows) => [:fruit-apple :fruit-pear]
                  (mapv #(apply str (::tui/children %)) rows) => ["Apple" "Pear"]
                  "wires the modal's :on-dismiss to the picker's :on-cancel"
                  (do ((tui/node-attr p :on-dismiss)) @cancelled) => true
                  "activating a row selects that option's value via :on-select"
                  (do (tui/activate! (first rows)) @selected) => :apple)))
