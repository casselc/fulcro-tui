...
(def tags
  ...
  #{:vbox :hbox :box :text :input :button :line :viewport})
...
(>defn- content-size
  ...
  (let [{::keys [tag attrs]} node
    ...
    (case tag
      ...
      (:vbox :box) (let [sizes (map child-size (::children node))]
...
(>defn place
  ...
  (let [{::keys [tag attrs children]} node
        ...
        content {:x (+ (:x rect) l)
                 ...
                 :h (max 0 (- (:h rect) t b))}]
    (case tag
      :vbox (assoc node ::rect rect ::children (place-stack :v content (mapv as-node children)))
...
(>defn frame->ansi
  "Returns the ANSI string to render the transition from buffer `prev` to buffer `next`, by diffing
   ...
   single atomic frame; otherwise no wrapper is added."
  [prev next opts]
  [(? ::buffer) ::buffer (s/keys :opt-un [::sync?]) => string?]
...
(>defn process-key!
  ...
  ([app key-event global-keymap]
   [any? map? (? map?) => any?]
   (let [node-tree    (current-node-tree app)
         k            (:key key-event)
     ...
     ;; re-resolve focus against the post-dispatch tree
     (resolve-focus! app (current-node-tree app))
     app)))
